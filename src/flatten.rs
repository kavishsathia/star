...
impl Flattener {
    ...
    pub fn scan_params(&mut self, params: &Vec<(String, Type, u32, std::rc::Rc<std::cell::RefCell<Option<String>>>)>) -> Vec<(String, Type, CaptureKind)> {
        ...
        for (name, ty, index, captured) in params.iter() {
            if let Some(field_name) = captured.borrow().as_ref() {
                self.captures.push((field_name.clone(), ty.clone(), CaptureKind::Index(*index)));
            }
        }
        self.captures.iter().cloned().collect()
    }

    pub fn flatten_stmt(&mut self, stmt: &AnalyzedStatement, captures: Vec<(String, Type, CaptureKind)>, prev: String) -> AnalyzedStatement {
        match stmt {
            AnalyzedStatement::Function { name, params, returns, body, captured, index, fn_index, locals } => {
                ...
                self.structs.push(AnalyzedStatement::Struct {
                    name: format!("{}", name),
                    fields: captures_to_pass_down.iter().map(|(n, t, _)| (n.clone(), t.clone())).collect(),
                });

                let outer_fields: Vec<(String, AnalyzedExpr)> = captures.iter().map(|(n, t, k)| (n.clone(), match k {
                    CaptureKind::Field => AnalyzedExpr {
                        expr: aast::Expr::Field {
                            object: Box::new(AnalyzedExpr {
                                expr: aast::Expr::Identifier { name: "captures".to_string(), index: Some(1) },
                                ty: Type {
                                    kind: TypeKind::Struct { name: format!("{}", name) },
                                    nullable: false,
                                    errorable: false,
                                },
                            }),
                    ...
                    CaptureKind::Index(idx) => AnalyzedExpr {
                        expr: aast::Expr::Identifier { name: n.clone(), index: Some(*idx) },
                        ty: t.clone(),
                    }
                })).collect();

                let struct_init = AnalyzedExpr { expr: aast::Expr::New {
                    name: format!("{}", prev),
                    fields: outer_fields,
                }, ty: Type {
                    kind: TypeKind::Struct { name: format!("{}", prev) },
                    nullable: false,
                    errorable: false,
                }};

                let analyzed_body: Vec<_> = body.iter().map(|s| self.flatten_stmt(s, captures_to_pass_down.clone(), name.clone())).collect();
            ...
            AnalyzedStatement::If { condition, then_block, else_block } => {
                let analyzed_then: Vec<_> = then_block.iter().map(|s| self.flatten_stmt(s, captures.clone(), prev.clone())).collect();
                let analyzed_else = else_block.as_ref().map(|stmts| {
                    let result: Vec<_> = stmts.iter().map(|s| self.flatten_stmt(s, captures.clone(), prev.clone())).collect();
                    result
                });
            ...
            AnalyzedStatement::While { condition, body } => {
                let analyzed_body: Vec<_> = body.iter().map(|s| self.flatten_stmt(s, captures.clone(), prev.clone())).collect();
            ...
            AnalyzedStatement::For { init, condition, update, body } => {
                let analyzed_init = Box::new(self.flatten_stmt(init, captures.clone(), prev.clone()));
                let analyzed_update = Box::new(self.flatten_stmt(update, captures.clone(), prev.clone()));
                let analyzed_body: Vec<_> = body.iter().map(|s| self.flatten_stmt(s, captures.clone(), prev.clone() )).collect();
